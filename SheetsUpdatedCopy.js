...
function Sheets() {
  ...
  const fetchDrawingCoordinates = async () => {
    try {
      const response = await ProjectFinder.get(
        `/api/coordinates/${project_id}/images/${image_id}`
      );
      const coordinatesMap = {};
      response.data.forEach((coordinates) => {
        coordinatesMap[coordinates.image_id] = JSON.parse(
          coordinates.coordinates
        );
      });
      setDrawingCoordinates(coordinatesMap);
    } catch (error) {
      console.error("Error fetching drawing coordinates", error);
    }
  };

  useEffect(() => {
    fetchImage();
    fetchDrawingCoordinates();
  }, [project_id]);
      ...
      <div>
        ...
        {images.map((imageUrl, index) => (
          <div key={index}>
            ...
            {drawingCoordinates[imageUrl.image_id] && (
              <Canvas
                width={200}
                height={150}
                drawingCoordinates={drawingCoordinates[imageUrl.image_id]}
              />
            )}
          </div>
        ))}
